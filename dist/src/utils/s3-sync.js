...
async function s3Sync({
  ...
  if (skippedFiles > 0) {
    console.log(import_chalk.default.gray(`Skipped uploading ${skippedFiles} unchanged files`));
  }
...
async function s3ListAll(aws, bucketName) {
  var _a;
  let result;
...
function findObjectsToDelete(existing, target) {
